    ...
    pub(crate) fn from(map_vec: Vec<String>, x_size: usize, y_size: usize) -> Grid {
        ...
        for (i, line) in map_vec.iter().enumerate() {
            for (j, next_char) in line.chars().enumerate() {
                match next_char {
                    node::WALL => {
                        map[i][j].node_state = NodeState::Wall;
                    }
                    node::DESTINATION => {
